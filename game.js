    ...
    class GameController {
        ...
        updateGameStatusUI() {
            ...
            if (!this.gameState.opponentConnected) {
                this.ui.updateStatus('等待对手连接...');
            } else if (GameLogic.isKingInCheck(this.gameState.board, this.gameState.currentTurn)) {
                this.ui.updateStatus(`${this.gameState.currentTurn === 'red' ? '红方' : '黑方'}被将军!`, this.gameState.currentTurn);
            } else if (this.gameState.isMyTurn()) {
                this.ui.updateStatus('轮到你走棋', this.gameState.currentTurn);
            } else {
                this.ui.updateStatus("等待对方走棋...", this.gameState.currentTurn);
            }
        ...
        async handlePresenceUpdate() {
            ...
            if (!wasConnected && this.gameState.opponentConnected) {
                console.log("对手已连接。");
                this.ui.updateStatus('游戏开始！', 'info');
        ...
        requestUndo() {
            ...
            this.ui.updateStatus("悔棋请求已发送...", 'wait');
        }
        ...
        handleUndoResponse(accepted) {
            if (accepted) {
                this.ui.updateStatus('对方同意了你的悔棋请求。', 'info');
                this.gameState.undoLastMove();
                this.ui.renderBoard(this.gameState.board);
            } else {
                this.ui.updateStatus('对方拒绝了你的悔棋请求。', 'info');
            }
            this.updateGameStatusUI();
        }

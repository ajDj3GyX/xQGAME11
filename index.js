document.addEventListener('DOMContentLoaded', () => {
    ...
    const showNotification = (message, type = 'error', duration = 5000) => {
        ...
        notification.innerHTML = `<i class="fa-solid ${type === 'error' ? 'fa-circle-exclamation' : 'fa-circle-check'}"></i> ${message}`;
    ...
    const createRoom = async () => {
        ...
        try {
            ...
            setLoadingState(ui.createRoomBtn, false, '创建新房间');
        }
    };
    ...
    const joinRoom = async () => {
        ...
        try {
            ...
            if (presence.length === 0) {
                ...
                const existingPlayer = presence[0].data.color;
                playerColor = existingPlayer === 'red' ? 'black' : 'red';
            } else {
                showNotification('房间已满，无法加入');
                setLoadingState(ui.joinRoomBtn, false, '加入');
                return;
            }
